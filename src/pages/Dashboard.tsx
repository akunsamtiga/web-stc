...
const Spark: React.FC<{ data: number[]; color: string }> = ({ data, color }) => {
  ...
  return (
    <div className="flex items-end gap-0.5 h-6 mt-2">
      {data.map((v, i) => (
        <div key={i}
          className={`flex-1 rounded-sm ${color} opacity-60 transition-all duration-500`}
...
const TrendPill: React.FC<{ curr: number; prev: number }> = ({ curr, prev }) => {
  if (prev === 0 && curr === 0)
    return <span className="inline-flex items-center gap-0.5 text-[10px] text-slate-400"><Minus size={9} />—</span>;
  if (prev === 0)
    return <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600"><ArrowUpRight size={9} />New</span>;
...
const StatCard: React.FC<{
  ...
  <div className="card p-4 hover:shadow transition-shadow duration-200">
  ...
  return (
    <div className="space-y-5 animate-fade-in pb-8">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={() => loadData(true)} disabled={refreshing} className="btn-secondary">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sub={`${stats.uniqueDevices} unique devices`}
        ...
        <StatCard
          ...
          value={stats.loginToday} label="Active Today"
          sub={`${stats.loginThisWeek} this week`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5 lg:col-span-2">
          <SectionHead
            ...
            right={
              <div className="text-right">
                <p className="text-lg font-bold text-slate-900 tabular-nums">{stats.loginThisWeek}</p>
                  ...
                  <div className="w-full flex flex-col justify-end flex-1">
                    <div
                      className={`w-full rounded-t-sm transition-all duration-700 ${
                        isToday ? 'bg-blue-500' : 'bg-slate-200 hover:bg-blue-200'
                      }`}
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-100 mt-4 pt-3 border-t border-slate-100">
            {[
              { l: "Today's rate", v: `${stats.todayRate}%` },
              { l: 'This week',    v: String(stats.loginThisWeek) },
              { l: 'Daily avg',    v: stats.dailyAvg.toFixed(1) },
            ].map(item => (
              <div key={item.l} className="text-center px-2">
        ...
        <div className="card p-5">
          <SectionHead
            icon={UserPlus} bg="bg-violet-50" color="text-violet-600"
              ...
              {stats.topActive.map((u, i) => {
                ...
                return (
                  <div key={u.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                    <span className="w-4 text-[10px] font-bold text-slate-300 text-right tabular-nums flex-shrink-0">
                      {i + 1}
                    </span>
          ...
          {config && (
            <div className="card p-4">
              ...
              <div className="space-y-2">
                {[
                  {
                    icon: ExternalLink, label: 'Registration URL',
                    url: config.registrationUrl,
                    bg: 'bg-blue-50 border-blue-100',
                    tc: 'text-blue-700',
                    btn: 'border-blue-200 hover:bg-blue-100 text-blue-700',
                  },
                  {
                    icon: MessageCircle, label: 'WhatsApp Support',
                    url: config.whatsappHelpUrl,
                    bg: 'bg-emerald-50 border-emerald-100',
                    tc: 'text-emerald-700',
                    btn: 'border-emerald-200 hover:bg-emerald-100 text-emerald-700',
                  },
                ].map(item => (
                  <div key={item.label} className={`p-3 rounded-lg border ${item.bg}`}>
                    ...
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => copy(item.url, item.label)}
                        className={`flex-1 inline-flex items-center justify-center gap-1 text-[11px] py-1 bg-white rounded border transition-colors ${item.btn}`}
                      >
                        <Copy size={9} /> Copy
                      </button>
                      <a
