...
export const Settings: React.FC = () => {
  ...
  const handleSave = async () => {
    if (!isSuperAdmin) {
      toast.error('Only super admins can update');
      return;
    }
  ...
  return (
    <div className="space-y-4 animate-fade-in max-w-3xl pb-6">
      <div className="mb-5">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">Settings</h1>
        <p className="text-sm text-slate-600">Manage application configuration</p>
      </div>
      ...
      {!isSuperAdmin && (
        <div className="card bg-yellow-50 border-yellow-200 p-3">
          <div className="flex items-start gap-2">
            ...
            <div>
              <h3 className="font-bold text-yellow-900 mb-1 text-sm">Read-Only Mode</h3>
      ...
      <div className="card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-slate-900">Registration URL</h2>
            <p className="text-xs sm:text-sm text-slate-600">Default link for new users</p>
          </div>
        </div>
            ...
            <div className="p-3 bg-green-50 rounded-xl border border-green-200">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs sm:text-sm text-green-700 break-all">{config.registrationUrl}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-slate-900">WhatsApp Support</h2>
            <p className="text-xs sm:text-sm text-slate-600">Contact link for support</p>
          </div>
        </div>
          ...
          {config && (
            <div className="p-3 bg-green-50 rounded-xl border border-green-200">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-green-800 mb-1">Current Active URL:</p>
                  <p className="text-xs sm:text-sm text-green-700 break-all">{config.whatsappHelpUrl}</p>
                </div>
              </div>
            </div>
          )}
      ...
      {isSuperAdmin && (
        <div className="flex flex-col sm:flex-row gap-2">
          <button
            ...
            className="btn-secondary disabled:opacity-50 h-10 text-sm"
          >
            Reset Changes
          </button>
          <button
            onClick={handleSave}
            disabled={!hasChanges || saving}
            className="btn-primary flex items-center justify-center gap-2 disabled:opacity-50 h-10 text-sm"
          >
            {saving ? (
              <>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-slate-700 space-y-1.5">
            ...
            <ul className="list-disc list-inside space-y-0.5 text-slate-600">
              ...
              <li>Test links using external icon</li>
            </ul>
          </div>
        </div>
      </div>
